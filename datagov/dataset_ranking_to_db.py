...
class RankedRequestManager:
    ...
    def get_dataset_info(self, ds_id: str, relative_path: str):
        ...
        if ds_id not in self.dataset_loader.model_manager.datasets:
            ...
            if ds_id not in self.dataset_loader.model_manager.datasets:
                raise ValueError(f"Dataset ID {ds_id} not found in the dataset loader {absolute_path}.")

        result = {
            "org": self.dataset_loader.model_manager.datasets[ds_id].organization.dg_name,
            "id": ds_id,
    ...
    def rank(self):
        ...
        for input_file in self.inputs:
            ...
            dataset_ranking.ranking_date = datetime.datetime.fromtimestamp(input_file.stat().st_ctime)
            ...
            if df.empty:
                ...
                for index, row in tqdm(df.iterrows(), total=df.shape[0], desc=f"Processing {input_file_name}"):
                    url = row.get("linkUrl")
                    if url and isinstance(url, str):
                        ...
                        publisher = None
                        if link_file_type:
                            publisher = row.get("customEvent:DATAGOV_dataset_publisher", None)
                            if publisher == "NO PUB":
                                publisher = "Unknown"
                        else:
                            filename = row.get("fileName", "")
                            if not filename:
                                raise ValueError(f"File name not found for download request in {input_file_name}")
                            ...
                            for resource in associated_resources:
                                ...
                                if ds_id:
                                    ...
                                    if ds_id not in datasets_encountered:
                                        ...
                                        if ds_id not in self.dataset_rankings[input_file_name]:
                                            self.dataset_rankings[input_file_name][ds_id] = self.get_dataset_info(ds_id, datasets[ds_id]["path"])
                                            self.dataset_rankings[input_file_name][ds_id]["count"] = event_count
                                        else:
                                            self.dataset_rankings[input_file_name][ds_id]["count"] += event_count
                            ...
                            if url_lower in ranking:
                                ...
                                for ds_id, ds_info in datasets.items():
                                    if ds_id not in ranking[url_lower]["datasets"]:
                                        ...
                                        for res_id, count in ds_info["resources"].items():
                                            if res_id in ranking[url_lower]["datasets"][ds_id]["resources"]:
                                                ranking[url_lower]["datasets"][ds_id]["resources"][res_id] += 1
                                            else:
                                                ranking[url_lower]["datasets"][ds_id]["resources"][res_id] = count
            ...
            self.dataset_rankings[input_file_name] = list(self.dataset_rankings[input_file_name].values())
        ...
        for input_file_name, ranking_datasets in self.dataset_rankings.items():
            ...
            progress = tqdm(total=len(ranking_datasets), desc=f"Building ranks for {input_file_name}...")

            ranking = rankings[input_file_name]

            rank = 0
            sorted_datasets = sorted(ranking_datasets, key=lambda x: int(x["count"]), reverse=True)
            for sorted_dataset in sorted_datasets:
                ...
                if not ds_id in dataset_db_mapping:
                    dataset = db.query(Dataset).filter(Dataset.dg_id == ds_id).options(load_only(Dataset.id)).one()
                    if not dataset:
